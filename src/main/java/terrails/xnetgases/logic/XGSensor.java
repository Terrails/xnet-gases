...
public class XGSensor {
    ...
    public boolean test(@Nullable TileEntity te, XGLogicConnectorSettings settings) {
        if (sensorMode == SensorMode.GAS) {
            return GasUtils.getGasHandlerFor(te, settings.getFacing())
                    .map(handler -> GasUtils.getGasHandlerFor(filter, null).map(filterHandler -> {
                        if (filterHandler.getGasTankCount() <= 0) {
                            return false;
                        }

                        Gas filterChemical = filterHandler.getGasInTank(0).getType();
